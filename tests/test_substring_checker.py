import tempfile
from src.utils.substring_checker import check_substrings, load_data


def test_small_substring_checker() -> None:
    strings = ("`t Hooft, Gerard", "a Zamayón, Pelagius", "`t Hooft, Gerard")
...
def test_small_substring_checker_array_with_indeces_to_ignore() -> None:
    ...
    result = tuple(check_substrings(string, strings, index) for index, string in enumerate(strings))
...
def test_substring_checker_array_longer() -> None:
    ...
    try:
        ...
        result_from_file = tuple(check_substrings(string, data, index) for index, string in enumerate(data))

        result_from_tuple = tuple(check_substrings(string, strings, index) for index, string in enumerate(strings))
